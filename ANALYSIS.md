...
- **Setup** (`startTestBattle`): friendly = living crew snapshotted with `hp`; hostile = the contract's
  `encounterId` enemies (`data/encounters.js` / `enemies.js`) or `TEST_HOSTILE_UNITS` fallback.
...
- **Round phases**: `roll → targeting → executing → enemy_turn → endRound`.
  ...
  - **Enemy turn**: each living enemy hits a random living friendly for `6 + rand(0..7)`, reduced by
    active `damage_reduction` buffs (min 1). Player vitals → 0 sets `world.gameOver = 'FLATLINE'` + defeat.
  - `endRound` ages buffs, regens pool, resets dice/rerolls, increments `round`.
...
- `enemies.js`/`encounters.js` are minimal (one enemy, one encounter); most combat uses `TEST_HOSTILE_UNITS`.
...
- No automated tests. The dev panel is the manual test harness.
