...
export const createRelationshipSlice = (set, get) => ({
  ...
  useFriendFavor: (friendId) =>
    set((state) => {
      ...
      if (eff.morale) {
        state.character.morale = clamp(0, 100, (state.character.morale ?? 50) + eff.morale);
      }
    ...
    set((state) => {
      ...
      for (const member of state.crew.members) {
        ...
        if (turn - member.lastBondTurn >= 5) {
          member.bond = clamp(BOND_MIN, BOND_MAX, (member.bond ?? 0) - Math.ceil(decayAmt));
      ...
      for (const [fid, friend] of Object.entries(state.relationship.friends)) {
        ...
        if (turn - friend.lastBondTurn >= 5) {
          friend.bond = clamp(BOND_MIN, BOND_MAX, (friend.bond ?? 0) - Math.ceil(decayAmt));
  ...
  devSetCrewBond: (memberId, value) =>
    set((state) => {
      ...
      if (!member) return;
      member.bond = clamp(BOND_MIN, BOND_MAX, Math.round(Number(value) || 0));
    }),

  devSetFriendBond: (friendId, value) =>
    set((state) => {
      if (!state.relationship.friends[friendId]) return;
      state.relationship.friends[friendId].bond = clamp(BOND_MIN, BOND_MAX, Math.round(Number(value) || 0));
    }),
});
